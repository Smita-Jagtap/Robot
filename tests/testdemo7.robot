...
Add Book into Library DataBase
    [Tags]    API
    &{req_body}=    Create Dictionary    name=${book_name}     isbn=982059    aisle=324059    author=rahulshetty
    ${response}=    POST    ${base_url}/Library/Addbook.php    json=${req_body}    verify=${False}    expected_status=200
    log    ${response.json()}
...
Get the Book Details which got added
    [Tags]    API
    ${get_response}=    GET    ${base_url}/Library/GetBook.php    params=ID=${book_id}    verify=${False}    expected_status=200
    log    ${get_response.json()}
    Should be Equal as Strings    ${book_name}    ${get_response.json()}[0][book_name]

Delete the Book from database
    &{delete_req}=    Create Dictionary    ID=${book_id}
    ${delete_resp}=    POST    ${base_url}/Library/DeleteBook.php    json=${delete_req}    verify=${False}    expected_status=200
